...
destRoot = fullfile(currentPath, 'results', 'ORL') ;
for i = 1 : classNum
...
%                     17 17 ];
para.imageSize = [ 48  48 ];
para.coordinate = [ 7  42 ; ...
                    25 25 ];
...
para.lambdac = 0.6 ;         % lambda = lambdac / sqrt(m) for ||E||_1

trainImageNum = 30;                  % train image number for each category
testImageNum  = 180 - trainImageNum; % test image number for each category
...
para.beta = 40;           % equal to the number of class 
...
[trainImages, testImages, trainTransform, testTransform] = get_images_ORL(imagePath, para, transformationInit);
