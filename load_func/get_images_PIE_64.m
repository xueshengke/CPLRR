function [trainImages, testImages, trainTransform, testTransform] = get_images_PIE_64( ...
...
load(fullfile(imagePath, 'PIE_64x64.mat'), 'PIEData');
imgSize  = para.imageSize;
classNum = para.classNum;

%% resize and normalize images
normPIE = cell(classNum, 1);
for i = 1 : classNum
    imageNum = size(PIEData{i}, 3);
    normPIE{i} = zeros(imgSize(1), imgSize(2), imageNum);
    tmp= PIEData{i} / 255;
    normPIE{i} = imresize(tmp, [imgSize(1) imgSize(2)]);
end
