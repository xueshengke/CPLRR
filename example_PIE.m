...
% load('PIE_32x32.mat', 'className');
load('PIE_64x64.mat', 'className');
userName = className;
classNum = numel(userName);

% output path
destDir = cell(classNum, 1);
destRoot = fullfile(currentPath, 'result', 'PIE') ;
for i = 1 : classNum
...
para.lambdac = 1*0.6 ;           % lambda = lambdac / sqrt(m) for ||E||_1

trainImageNum = 25;                 % train image number for each category
testImageNum  = 45 - trainImageNum; % test image number for each category
para.trainImageNum = trainImageNum;
para.testImageNum  = testImageNum;
para.classNum = classNum;

para.noise_ratio = 0.15;   % 0, 0.1, 0.2, 0.3
para.alpha = 0.9;
para.gamma = 15;           % 5, 15, 25, 35
...
% [trainImages, testImages, trainTransform, testTransform] = get_images_PIE_32(imagePath, para, transformationInit);
[trainImages, testImages, trainTransform, testTransform] = get_images_PIE_64(imagePath, para, transformationInit);
...
%% record test results
outputFileName = fullfile(destRoot, 'PIE_parameters.txt'); 
